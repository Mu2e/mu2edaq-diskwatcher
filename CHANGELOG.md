...
- **Peers found by discovery.** `peers:` may now be a mapping with `static:`
  ...
  **`probe:`** — hosts (or `host:port`) also asked directly, by unicast, on
  every scan. Added after measuring the DAQ network: all nodes on one
  `/24`, every responder up, yet a multicast scan from `mu2e-mgr-01` found
  only itself, one from `mu2e-trk-03` found only the nodes on its own switch,
  and one from `mu2e-trk-11` found a third island. That is IGMP snooping with
  no querier. A unicast datagram to any node's port 28999 was answered at
  once, so the same DISCOVER query is now also sent to each listed host;
  answers from both paths are merged by responder id, a host that cannot be
  sent to is reported, and `/api/peers` shows `multicast_replies`,
  `probe_replies` and `probe_errors`.

  **`config/mu2e-diskwatcher-mgr-01.yaml`**, the aggregator, generated by
  `tools/make-node-configs.sh --aggregator` (default `mu2e-mgr-01`) from
  `config/nodes/aggregator.template.yaml`: every node under `static:` and
  under `probe:`, discovery on, its own six areas and two logs watched. It
  exists because the aggregator had been running on a locally edited copy of
  the shipped example config, and a `git pull` replaced that with the shipped
  file, which has discovery off.
